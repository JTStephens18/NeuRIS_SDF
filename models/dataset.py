...
from models.read_write_model import read_images_binary, qvec2rotmat, read_points3D_binary
...
class Dataset:
    ...
    def __init__(self, conf):
        ...
        images_bin_path = os.path.join(self.data_dir, 'images.bin') 
        c2wmatrices = self.get_camera_to_world_matrices(images_bin_path)

        first_img_id = next(iter(c2wmatrices))
        print(f"\nCamera-to-world matrix for image ID {first_img_id}:")
        print(c2wmatrices[first_img_id])
        ...
        sample_img = self.read_img(images_lis[0], self.resolution_level)

        images_shape = (self.n_images,) + sample_img.shape
        print("Images shape ", images_shape)

        self.images_np = np.stack([self.read_img(im_name, self.resolution_level) for im_name in images_lis]).astype(np.float32) / 256.0
        # print("Images np shape ", self.images_np.shape)
        ...
        self.world_mats_np = self.format_c2w_as_list(c2wmatrices)

        print("After world mat")
        ...
        if self.estimate_scale_mat:
            ...
            points_binary_path = os.path.join(self.data_dir, 'points3D.bin') 
            self.scale_mats_np = self.create_normalization_matrix(points_binary_path)
        ...
        self.masks  = torch.from_numpy(self.masks_np.astype(np.float32)).cuda()   # n_images, H, W, 3   # Save GPU memory
    ...
    def initialize_patchmatch(self):
        ...
        if self.denoise_gray_image:
            ...
            if not checkExistence(dir_denoise) and len(get_files_stem(dir_denoise, '.png'))==0:
                ...
                stems_imgs = get_files_stem(f'{self.data_dir}/image', '.jpg')
    ...
    def get_camera_to_world_matrices(self, images_path):
        """
        Reads camera poses from an images.bin file and computes the 4x4
        camera-to-world transformation matrix for each image.

        Args:
            images_bin_path: Path to the COLMAP images.bin file.

        Returns:
            A dictionary mapping image_id to its 4x4 camera-to-world matrix (np.ndarray).
        """
            
        # Read the images.bin file to get camera poses
        ...
        for image_id, image in images.items():
            # Extract rotation (as a quaternion) and translation vectors
            qvec = image.qvec
            tvec = image.tvec

            # 1. Convert quaternion to a 3x3 rotation matrix (world-to-camera)
            # This function is available in read_write_model.py
            R_world_to_cam = qvec2rotmat(qvec)

            # 2. Invert the transformation to get camera-to-world
            # The inverse of a rotation matrix is its transpose
            R_cam_to_world = R_world_to_cam.T
            
            # The inverse translation is -R_transpose * t
            t_cam_to_world = -np.dot(R_cam_to_world, tvec)

            # 3. Assemble the 4x4 camera-to-world matrix
            # Create a 4x4 identity matrix
            cam_to_world = np.eye(4)
            
            # Place the rotation matrix in the top-left 3x3 block
            cam_to_world[0:3, 0:3] = R_cam_to_world
            
            # Place the translation vector in the last column
            cam_to_world[0:3, 3] = t_cam_to_world
            
            cam_to_world_matrices[image_id] = cam_to_world
            
        return cam_to_world_matrices
    
    def format_c2w_as_list(self, c2w_matrices_dict):
        """
        Converts a dictionary of camera-to-world matrices into an
        ordered list of float32 matrices.

        Args:
            c2w_matrices_dict: A dictionary mapping image_id to its 
                            4x4 camera-to-world matrix.

        Returns:
            A list of 4x4 camera-to-world matrices, sorted by image_id
            and cast to np.float32.
        """
        # Get the image IDs and sort them to ensure a consistent order
        sorted_ids = sorted(c2w_matrices_dict.keys())
        
        # Create the list by retrieving each matrix in the sorted order
        # and converting its type to float32
        world_mats_list = [c2w_matrices_dict[id].astype(np.float32) for id in sorted_ids]
        
        return world_mats_list
    
    def create_normalization_matrix(self, points_path):
        """
        Creates a normalization matrix from a COLMAP points3D.txt file.

        This matrix transforms the scene so that the point cloud is centered
        at the origin and fits within a unit cube.

        Args:
            points_file_path: The file path to the COLMAP points3D.txt file.

        Returns:
            A 4x4 numpy array representing the normalization matrix.
        """
        print("Creating normalization matrix")

        points3D_obj = read_points3D_binary(points_path)

        xyz_coords_list = [p.xyz for p in points3D_obj.values()]

        # Create a single (N, 3) NumPy array from the list of coordinates
        points = np.array(xyz_coords_list)

        # 2. Compute the scene's bounding box
        min_coords = np.min(points, axis=0)
        max_coords = np.max(points, axis=0)
        
        # 3. Determine the scene's center
        scene_center = (min_coords + max_coords) / 2.0
        
        # 4. Determine the scale factor
        scene_extent = np.max(max_coords - min_coords)
        scale_factor = 1.0 / scene_extent
        
        # 5. Construct the normalization matrix
        # This matrix first translates the scene to the origin, then scales it.
        
        # Translation matrix to move the scene center to the origin
        translation_matrix = np.identity(4)
        translation_matrix[:3, 3] = -scene_center
        
        # Scaling matrix to fit the scene in a unit cube
        ...
        # The final normalization matrix is the product of scaling and translation
        norm_matrix = scaling_matrix @ translation_matrix

        norm_matrix_list = [norm_matrix.astype(np.float32) for _ in range(self.n_images)]

        return norm_matrix_list
